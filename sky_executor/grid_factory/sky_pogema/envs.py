...
class PogemaBase(gymnasium.Env):
    ...
    def __init__(self, grid_config: GridConfig = GridConfig()):
        ...
        self.action_space: gymnasium.spaces.Discrete = gymnasium.spaces.Discrete(
            len(self.grid_config.MOVES)
        )
        self._multi_action_sampler = ActionsSampler(
            self.action_space.n, seed=self.grid_config.seed
        )
    ...
    def sample_actions(self):
        ...
        :return:
        """
        return self._multi_action_sampler.sample_actions(
            dim=self.grid_config.num_agents
        )
...
class Pogema(PogemaBase):
    def __init__(self, grid_config=GridConfig(num_agents=2)):
        ...
        if self.grid_config.observation_type == "default":
            self.observation_space = gymnasium.spaces.Box(
                -1.0, 1.0, shape=(3, full_size, full_size)
            )
            ...
            self.observation_space: gymnasium.spaces.Dict = gymnasium.spaces.Dict(
                ...
                target_xy=gymnasium.spaces.Box(
            ...
            raise ValueError(
                f"Unknown observation type: {self.grid.config.observation_type}"
            )
    ...
    def update_was_on_goal(self):
        self.was_on_goal = [
            self.grid.on_goal(agent_idx) and self.grid.is_active[agent_idx]
            for agent_idx in range(self.grid_config.num_agents)
        ]
        ...
        if self.grid.config.collision_system == "priority":
            ...
            for agent_idx, (x, y) in enumerate(agents_xy):
                if self.grid.is_active[agent_idx]:
                    ...
                    used_cells[x + dx, y + dy] = (
                        "blocked" if (x + dx, y + dy) in used_cells else "visited"
                    )
            ...
            for agent_idx, (x, y) in enumerate(agents_xy):
                if self.grid.is_active[agent_idx]:
                    ...
                    if dx != 0 or dy != 0:
                        used_edges.setdefault((x + dx, y + dy, x, y), []).append(
                            agent_idx
                        )
            for agent_idx, (x, y) in enumerate(agents_xy):
                if self.grid.is_active[agent_idx]:
            ...
            for agent_idx in reversed(range(len(agents_xy))):
                ...
                if self.grid.is_active[agent_idx]:
                    dx, dy = self.grid.config.MOVES[actions[agent_idx]]
                    if len(used_cells[x + dx, y + dy]) > 1 or self.grid.has_obstacle(
                        x + dx, y + dy
                    ):
                        actions, used_cells = self._revert_action(
                            agent_idx, used_cells, (x + dx, y + dy), actions
                        )
            for agent_idx in range(self.grid_config.num_agents):
                if self.grid.is_active[agent_idx]:
            ...
            raise ValueError(
                "Unknown collision system: {}".format(self.grid.config.collision_system)
            )
    ...
    def get_agents_xy(self, only_active=False, ignore_borders=False):
        return self.grid.get_agents_xy(
            only_active=only_active, ignore_borders=ignore_borders
        )
...
class PogemaLifeLong(Pogema):
    ...
    def _initialize_grid(self):
        ...
        seeds = main_rng.integers(
            np.iinfo(np.int32).max, size=self.grid_config.num_agents
        )
    ...
    def get_lifelong_targets_xy(self, ignore_borders=False):
        if self.has_custom_sequences:
            if ignore_borders:
                ...
                return [
                    [
                        [
                            x + self.grid_config.obs_radius,
                            y + self.grid_config.obs_radius,
                        ]
                        for x, y in sequence
                    ]
        ...
        for agent_idx in range(self.grid_config.num_agents):
            ...
            initial_target = self.get_targets_xy(ignore_borders=ignore_borders)[
                agent_idx
            ]
            agent_sequence.append(initial_target)
            current_pos = initial_target
            total_distance = abs(start_pos[0] - initial_target[0]) + abs(
                start_pos[1] - initial_target[1]
            )

            while total_distance < self.grid_config.max_episode_steps:
                if ignore_borders:
                    generator_pos = (
                        current_pos[0] + self.grid_config.obs_radius,
                ...
                total_distance += abs(current_pos[0] - goal_coords[0]) + abs(
                    current_pos[1] - goal_coords[1]
                )
    ...
    def _generate_new_target(self, agent_idx):
        if self.has_custom_sequences:
            ...
            self.current_goal_indices[agent_idx] = (current_idx + 1) % len(
                agent_targets
            )

            if (
                self.current_goal_indices[agent_idx] == 0
                and current_idx == len(agent_targets) - 1
            ):
                warnings.warn(
            ...
            return (
                next_target[0] + self.grid_config.obs_radius,
...
class PogemaCoopFinish(Pogema):
    ...
    def step(self, action: list):
        ...
        truncated = [False] * self.grid_config.num_agents
        rewards = [
            1.0 if is_task_solved else 0.0 for _ in range(self.grid_config.num_agents)
        ]
        return obs, rewards, terminated, truncated, infos
